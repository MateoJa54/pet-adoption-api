...
export default function () {
  ...
  if (token) {
    res = http.post(
      `${baseUrl}/shelters`,
      JSON.stringify({
        name: `Shelter ${randomString(5)}`,
        location: `${Math.floor(Math.random() * 9999)} Shelter Ave`,
        contactEmail: `${randomString(8)}@gmail.com`,
    ...
    check(res, {
      'crear shelter exitoso': (r) => r.status === 201,
    });

    //obtener shelters
    res = http.get(`${baseUrl}/shelters`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    check(res, {
      'obtener shelters exitoso': (r) => r.status === 200,
    });
  }
